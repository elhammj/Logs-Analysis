...
query_to_answer_1 = """select title, count(title) as views
from articles,log where log.status = '200 OK' and
log.path = concat('/article/',articles.slug)
group by title order by views desc limit 3;"""
query_to_answer_2 = """select name, count(author) as views
from articles,log, authors where log.status = '200 OK'
and log.path = concat('/article/',articles.slug) and
articles.author = authors.id group by name order by views desc;"""
query_to_answer_3 = """select to_char(day,'Mon DD,YYYY'), error_percentage
from (select date_trunc('day', time) as day,round(100.0*sum(
case log.status when '404 NOT FOUND'then 1 else 0 end)
/count(log.status),3) as error_percentage
from log  group by 1 order by 1) subq where error_percentage > 1;"""
